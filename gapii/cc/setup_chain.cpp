...
void InitializeLayer(
  ...
  GAPID_INFO("InitializeLayer(%p, %p)", layer_id, get_next_layer_proc_address);
  g_layer_id = layer_id;
  g_next_layer_proc_addr = get_next_layer_proc_address;
  core::GetGlesProcAddress = &get_next_gles_proc_address;
}

void* GetLayerProcAddress(const char* name, void* next) {
  GAPID_INFO("GetLayerProcAddress(%s, %p)", name, next);
