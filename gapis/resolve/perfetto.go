...
func (r *PerfettoResolvable) Resolve(ctx context.Context) (interface{}, error) {
	...
	d, err := Device(ctx, r.Config.ReplayDevice, r.Config)
	if err != nil {
		return nil, err
	}
	...
	for _, a := range c.APIs {
		if pf, ok := a.(replay.Profiler); ok && pf.SupportsPerfetto(ctx, d) {
			...
			r, err := capture.Import(ctx, c.Name()+"_perfetto", src)
			if err != nil {
				return nil, err
			}
	...
	return nil, errors.New("The capture and/or replay device do not support Perfetto")
}
