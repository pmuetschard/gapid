...
processor new_processor(opt_mode opt, uint64_t window_size_ns) {
  ptp::Config config;

  switch (opt) {
    case kMinLatency:
      config.optimization_mode = ptp::OptimizationMode::kMinLatency;
      break;
    default:
      config.optimization_mode = ptp::OptimizationMode::kMaxBandwidth;
  }
