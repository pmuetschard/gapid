...
static void trace_write(VkQueue queue, uint32_t queue_idx, uint64_t start_ts, uint64_t end_ts, const char* label) {
  static pid_t pid = getpid();

  send_event(pid, reinterpret_cast<uintptr_t>(queue), queue_idx, start_ts, end_ts, label);
}
...
static void startListeningThread(const DeviceData& deviceObj, QueueData* qd) {
  ...
  qd->thr = std::thread([qd, waitforfences, freecommandbuffer,
    ...
    while (true) {
      ...
      trace_write(qd->queue, qd->queue_family_index << 16 | qd->queue_index, begin, end, buff);
