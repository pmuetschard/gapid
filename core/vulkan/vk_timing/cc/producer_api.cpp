...
typedef void (*PFN_send_event)(int32_t, uint64_t, uint32_t, uint64_t, uint64_t, const char*);

}

void send_event(int32_t pid, uint64_t queue_id, uint32_t queue_idx, uint64_t start_ts, uint64_t end_ts, const char* label) {
  static PFN_send_event se = (PFN_send_event)dlsym(getLibProducer(), "send_event");
  if (se != nullptr) {
